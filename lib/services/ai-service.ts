...
import { getStylePrompts } from '../config/conversation-prompts'
...
export class AIService extends BaseService {
  ...
  private async buildSystemPrompt(tone: string, matchName?: string, otherInfo?: string): Promise<string> {
    const stylePrompts = await getStylePrompts();
    let basePrompt = stylePrompts[tone] || stylePrompts.casual
  ...
  async generateReplies(request: GenerateReplyRequest): Promise<string[]> {
    return this.handleServiceError('generateReplies', async () => {
      this.log('debug', 'Generating replies', {
      ...
      const systemPrompt = await this.buildSystemPrompt(request.tone, request.matchName, request.otherInfo)
        ...
        try {
          const response = await fetch(`${this.baseURL}/chat/completions`, {
            ...
            body: JSON.stringify({
              ...
              messages: messages,
              temperature: this.getTemperatureByTone(request.tone),
              max_tokens: 150
            })
          })
          ...
          if (!response.ok) {
            ...
            this.log('error', 'OpenAI API error response', {
          ...
          this.log('debug', 'Successfully generated replies', {
          ...
          this.log('error', 'GPT Service Error in generateReplies', {
...
- Consider the best direction for the conversation to develop`
      ...
      try {
        ...
        this.log('error', 'GPT Service Error in regenerateReply', error as Error)
        throw error
      }
    })
  }
  ...
  async analyzeScreenshot(request: AnalyzeImageRequest): Promise<AnalyzeImageResponse> {
    return this.handleServiceError('analyzeScreenshot', async () => {
...
- Return only the JSON array without any additional explanation or text.`
      ...
      try {
        const response = await fetch(`${this.baseURL}/chat/completions`, {
          ...
          body: JSON.stringify({
            ...
            temperature: this.getTemperatureByTone(),
            max_tokens: 1000
          })
        })
        ...
        try {
          ...
          this.log('error', '解析对话内容失败', parseError)
          return {
            conversation: [],
            success: false,
        ...
        this.log('error', 'Screenshot Analysis Error', error)
        return {
          conversation: [],
          success: false,
          error: "分析截图失败，请重试"
  ...
  async analyzeScreenshotAndGenerate(request: AnalyzeAndGenerateRequest): Promise<AnalyzeAndGenerateResponse> {
    return this.handleServiceError('analyzeScreenshotAndGenerate', async () => {
      try {
        // First analyze screenshot to extract conversation
        ...
        // If no conversation content, return error
        ...
        this.log('error', 'Analyze and Generate Error', error)
        return {
          conversation: [],
          replies: [],
          success: false,
